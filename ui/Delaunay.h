...
{
	PointF() : X(0), Y(0), Id(-1)	{}
	PointF(const PointF& p) : X(p.X), Y(p.Y), Id(p.Id)	{}
	PointF(REAL x, REAL y, int i = -1) : X(x), Y(y), Id(i)	{}
...
{
public:
	vertex()					: m_Pnt(0.0F, 0.0F, -1)			{}
	vertex(const vertex& v)		: m_Pnt(v.m_Pnt)			{}
	vertex(const PointF& pnt)	: m_Pnt(pnt)				{}
	vertex(REAL x, REAL y, int i)		: m_Pnt(x, y, i)			{}
	vertex(int x, int y, int i)		: m_Pnt((REAL) x, (REAL) y, i)	{}
	...
	void SetId(int id)		{m_Pnt.Id = id;}
	int GetId()	const	{return m_Pnt.Id;}
