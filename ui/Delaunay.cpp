...
void Delaunay::Triangulate(const vertexSet& vertices, triangleSet& output)
{
	...
	vSuper[0] = vertex(xMin - dy * sqrt3 / 3.0F, yMin, -1);	// Simple highschool geometry, believe me.
	vSuper[1] = vertex(xMax + dy * sqrt3 / 3.0F, yMin, -2);
	vSuper[2] = vertex((xMin + xMax) * 0.5F, yMax + dx * sqrt3 * 0.5F, -3);

	std::vector<triangle> workset;
	...
	for (itVertex = vertices.begin(); itVertex != vertices.end(); itVertex++)
	{
		...
		auto itEnd = remove_if(workset.begin(), workset.end(), triangleIsCompleted(itVertex, output, vSuper));
